...
class AgentHaloToolBase(abc.ABC):
    """
    An AgentHalo Tool being used by AgentHalo Agents.
    """
    ...
    def _get_output_type_description(cls) -> Optional[str]:
        ...
        if issubclass(cls.output_type, BaseModel):
            # could add additional hints after the schema for AgentHaloToolInput class? or object docstring?
            return (
...
class AgentHaloToSmolAgentsToolAdapter:
    """Adapter class to convert AgentHaloToolBase instances to smolagents Tool instances."""

    @classmethod
    def adapt(cls, agenthalo_tool: AgentHaloToolBase) -> Tool:
        tool = Tool()

        tool.name = agenthalo_tool.name
        tool.description = agenthalo_tool.description
        tool.inputs = cls._construct_smolagents_inputs(agenthalo_tool)
        tool.output_type = cls._get_smolagents_type(agenthalo_tool.output_type)
        tool.forward = agenthalo_tool.forward

        return tool

    @classmethod
    def _construct_smolagents_inputs(cls, agenthalo_tool: AgentHaloToolBase) -> Dict[str, Any]:
        hints = get_type_hints(agenthalo_tool.forward)

        inputs = {
            name: {"description": description, "type": cls._get_smolagents_type(hints[name])}
            for name, description in agenthalo_tool.inputs_descriptions.items()
        }
        return inputs
