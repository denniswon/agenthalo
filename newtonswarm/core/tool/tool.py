...
class NewtonSwarmToolBase(abc.ABC):
    """
    An NewtonSwarm Tool being used by NewtonSwarm Agents.
    """
    ...
    def _get_output_type_description(cls) -> Optional[str]:
        ...
        if issubclass(cls.output_type, BaseModel):
            # could add additional hints after the schema for NewtonSwarmToolInput class? or object docstring?
            return (
...
class NewtonSwarmToSmolAgentsToolAdapter:
    """Adapter class to convert NewtonSwarmToolBase instances to smolagents Tool instances."""

    @classmethod
    def adapt(cls, newtonswarm_tool: NewtonSwarmToolBase) -> Tool:
        tool = Tool()

        tool.name = newtonswarm_tool.name
        tool.description = newtonswarm_tool.description
        tool.inputs = cls._construct_smolagents_inputs(newtonswarm_tool)
        tool.output_type = cls._get_smolagents_type(newtonswarm_tool.output_type)
        tool.forward = newtonswarm_tool.forward

        return tool

    @classmethod
    def _construct_smolagents_inputs(cls, newtonswarm_tool: NewtonSwarmToolBase) -> Dict[str, Any]:
        hints = get_type_hints(newtonswarm_tool.forward)

        inputs = {
            name: {"description": description, "type": cls._get_smolagents_type(hints[name])}
            for name, description in newtonswarm_tool.inputs_descriptions.items()
        }
        return inputs
